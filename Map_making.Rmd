...
Sites$Habitat<-factor(Sites$Habitat,levels=c("Savanna", "Intermediate", "Rainforest"))
# plot map
mapwbase<-ggmap(myMap) +geom_point(data=Sites, aes(x=lon, y=lat,fill = Habitat), shape = 23,size=10) + #c(7,7,7,7,7))+#,3.5,3.5,3.5))+ #,fill=cols
 ...
 geom_text(data = Sites, aes(x=lon+0.01, y=lat, label = c("Pennyweight (PW) \n","\nStation Creek (SC)","  Mt. Lewis (MS)","JCU","  Rainforest\nRescue (RR)")),size = 12,hjust = "left",fontface = "bold") + 
