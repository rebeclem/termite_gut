...
shan2<-shan + geom_boxplot(aes(fill = Site1),outlier.colour = "black", outlier.size = 1,show.legend = FALSE)+ geom_jitter(size=1,shape=1)+labs(y = "Shannon diversity",x="")+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+ scale_color_brewer(palette="Dark2")+theme_classic()
...
#Diversity of genera
gen_observed<- ggplot(diversityNORM_genus, aes(factor(t_sp1), Observed))
...
gen_shan <- ggplot(diversityNORM_genus, aes(factor(t_sp1), Shannon))
...
#Genus comparisons
t1_genus<-lmer(Observed~t_sp1+Site1+ (1|Sequencing_group), diversityNORM_genus)
summary(t1_genus)
confint(t1_genus)
t1_shan_genus<-lmer(Shannon~t_sp1+Site1+ (1|Sequencing_group), diversityNORM_genus)
...
```{r}
microdiv<-diversityNORM_Q1[diversityNORM_Q1$t_sp=="M_serratus",]
...
t1_m<-lm(Observed~t_sp+Site1, microdiv)
summary(t1_m)
t1_m_shan<-lm(Shannon~t_sp+Site1, microdiv)
summary(t1_m_shan)
...
ggsave("~/Box/termite_microbiome/Dada2/Jan2022/plots/NMDS_ps7_Unifrac.png")
...
phyloseq::plot_ordination(ps_set1, ord.nmds.bray_all_set1, color="t_sp", title="Bray NMDS",label="X.NAME")
...
phyloseq::plot_ordination(ps_set2, ord.nmds.bray_all_set2, color="t_sp", title="Bray NMDS",label="X.NAME")
...
phyloseq::plot_ordination(ps.bygenus.tr, ord.nmds.bray_genus, color="Site2",shape="Sequencing_group", title="Bray NMDS")
...
Mserratus<-subset_samples(m_serratus1,t_sp=="M_serratus")
MspG<-subset_samples(m_serratus1,t_sp=="M_sp_G")
...
m_serratus_plot<-phyloseq::plot_ordination(m_serratus, ord.nmds.bray_micro, color="Site1", title="Microcerotermes Bray NMDS genus")
...
phyloseq::plot_ordination(m_serratus2, ord.nmds.bray_micro2, color="Site1", title="Microcerotermes Bray NMDS ps7")
ord.nmds.bray_micro.tr <- ordinate(m_serratus1, method="NMDS", distance="bray")
phyloseq::plot_ordination(m_serratus1, ord.nmds.bray_micro.tr, color="Site1", title="Microcerotermes Bray NMDS transformed")+ stat_ellipse(type="norm", linetype=1) +  theme_bw()
...
phyloseq::plot_ordination(a_laurensis1, ord.nmds.bray_ami1, color="Site1",shape="Sequencing_group", title="Amitermes Bray NMDS transformed")
...
a_bothsites<-ggplot(a_model,aes(sampledist,commdist)) + geom_point(aes(color=samesite))+ggtitle("Amitermes laurensis")+geom_smooth(method='lm')+theme_classic() #+stat_summary(fun.data=mean_cl_normal) +
...
c_bothsites<-ggplot(c_model,aes(sampledist,commdist)) +geom_point(aes(color=samesite))+ggtitle("Coptotermes acinaciformis")+geom_smooth(method='lm')+theme_classic()
...
ggplot(m_model,aes(sampledist,commdist,color=samespec)) +geom_point(aes(shape=samesite))+ggtitle("Microcerotermes serratus")+geom_smooth(method="lm")
...
m1_bothsites<-ggplot(ms_model,aes(sampledist,commdist,shape=samespec)) +geom_point(aes(color=samesite))+ggtitle("Microcerotermes serratus")+geom_smooth(method="lm")+theme_classic()
...
m2_bothsites<-ggplot(mG_model,aes(sampledist,commdist,shape=samespec)) +geom_point(aes(shape=samespec,color=samesite))+ggtitle("Microcerotermes sp G")+geom_smooth(method="lm")+theme_classic()
...
n_bothsites<-ggplot(n_model,aes(sampledist,commdist)) +geom_point(aes(color=samesite))+ggtitle("Nasutitermes magnus")+geom_smooth(method='lm')+theme_classic()
...
apw_plot<-ggplot(a_pw_model,aes(sampledist,commdist)) +geom_point()+ggtitle("Sav1 Amitermes")+ geom_smooth(method='lm')
...
asc_plot<-ggplot(a_sc_model,aes(sampledist,commdist)) +geom_point()+ggtitle("Sav2 Amitermes")+ geom_smooth(method='lm')
...
npw_plot<-ggplot(n_pw_model,aes(sampledist,commdist)) +geom_point()+ggtitle("Sav1 Nasutitermes")+ geom_smooth(method='lm')
...
nsc_plot<-ggplot(n_sc_model,aes(sampledist,commdist)) +geom_point()+ggtitle("Sav2 Nasutitermes")+ geom_smooth(method='lm')
...
cpw_plot<-ggplot(c_pw_model,aes(sampledist,commdist)) +geom_point()+ggtitle("Sav1 Coptotermes")+ geom_smooth(method='lm')
...
csc_plot<-ggplot(c_sc_model,aes(sampledist,commdist)) +geom_point()+ggtitle("Sav2 Coptotermes")+ geom_smooth(method='lm')
...
phyloseq::plot_ordination(ps9, ord.nmds.bray_fun, color="t_sp",shape="Site2", title="Bray NMDS Pathway")+   theme_bw()#stat_ellipse(type="norm", linetype=1) +
...
dim(merged_by_ss.df_fun) #264x11
...
sigtab_sav_phylum[,c("Phylum","log2FoldChange")]
...
keep_percent<-.008
ps.any.f <- filter_taxa(ps.any.tr, function (x) mean(x) > keep_percent, TRUE) 
  patho.by_any_plot <- plot_bar(ps.any.f, "Sample", "Abundance","EC")
